...
/**
 * Find all ItemDetails based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of ItemDetails per page for pagination.
 ...
 * @returns {Object} - The result containing ItemDetails, metadata, and filters.
 */
const findAll = async ({
  sort,
  fields,
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.itemDetailsAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.itemDetailsAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch itemDetails from the database
  let itemDetails = await ItemDetails.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    itemDetails = await getPopulatedFields(populatedFields, itemDetails);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    itemDetails = selectFields(itemDetails, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    itemDetails = itemDetails.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await ItemDetails.count(searchQuery);
  ...
  let finalItems = [];
  if (query.selectedFields.length > 0) {
    finalItems = itemDetails.map((itemDetail) => ({
      id: itemDetail.id,
      data: { ...itemDetail },
    }));
  } else {
