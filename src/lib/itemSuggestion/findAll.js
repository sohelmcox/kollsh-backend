...
/**
 * Find all ItemSuggestion based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of ItemSuggestion per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(
      sort,
      config.itemSuggestionAllowedSorByFields,
    ),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.itemSuggestionAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch itemSuggestions from the database
  let itemSuggestions = await ItemSuggestion.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    itemSuggestions = await getPopulatedFields(
      populatedFields,
      itemSuggestions,
    );
  }
  ...
  if (query.selectedFields.length > 0) {
    itemSuggestions = selectFields(itemSuggestions, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    itemSuggestions = itemSuggestions.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await ItemSuggestion.count(searchQuery);
  ...
  const paginationResponse = {
    ...
    limit: query.limit,
    skip: query.skip,
    totalEntities: itemSuggestions.length,
    totalCount,
  };
  const filters = {
    locale: query.locale,
  ...
  let finalItems = [];
  if (query.selectedFields.length > 0) {
    finalItems = itemSuggestions.map((itemSuggestion) => ({
      id: itemSuggestion.id,
      data: { ...itemSuggestion },
    }));
  } else {
    finalItems = itemSuggestions.map((itemSuggestion) => ({
      id: itemSuggestion.id,
      data: { ...itemSuggestion._doc },
    }));
  }
