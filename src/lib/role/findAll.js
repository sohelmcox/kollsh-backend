...
/**
 * Find all Roles based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of Roles per page for pagination.
 ...
 * @returns {Object} - The result containing Roles, metadata, and filters.
 */
const findAll = async ({
  sort,
  fields,
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.roleAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch roles from the database
  let roles = await Role.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    roles = await getPopulatedFields(populatedFields, roles);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    roles = selectFields(roles, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    roles = roles.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await Role.count(searchQuery);
