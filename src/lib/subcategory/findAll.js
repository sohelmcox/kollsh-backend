...
/**
 * Find all Subcategory based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of Subcategory per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.subcategoryAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.subcategoryAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch subcategories from the database
  let subcategories = await Subcategory.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    subcategories = await getPopulatedFields(populatedFields, subcategories);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    subcategories = selectFields(subcategories, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    subcategories = subcategories.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await Subcategory.count(searchQuery);
  ...
  let finalItems = [];
  if (query.selectedFields.length > 0) {
    finalItems = subcategories.map((subcategory) => ({
      id: subcategory.id,
      data: { ...subcategory },
    }));
  } else {
