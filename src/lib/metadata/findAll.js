...
/**
 * Find all Metadata based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of Metadata per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.metadataAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.metadataAllowedSearchFields,
    );
  }
  ...
  if (query.selectedFields.length > 0) {
    metadata = selectFields(metadata, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    metadata = metadata.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await Metadata.count(searchQuery);
  ...
  let finalItems = [];
  if (query.selectedFields.length > 0) {
    finalItems = metadata.map((metadata) => ({
      id: metadata.id,
      data: { ...metadata },
    }));
  } else {
