...
/**
 * Find all Reply based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of Reply per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.replyAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.replyAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch replies from the database
  let replies = await Reply.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    replies = await getPopulatedFields(populatedFields, replies);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    replies = selectFields(replies, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    replies = replies.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await Reply.count(searchQuery);
