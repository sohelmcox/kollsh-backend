...
/**
 * Find all User based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of User per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.userAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.userAllowedSearchFields,
    );
  }
  ...
  if (populatedFields.length > 0) {
    users = await getPopulatedFields(populatedFields, users);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    users = selectFields(users, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    users = users.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await User.count(searchQuery);
