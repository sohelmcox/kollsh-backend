...
/**
 * Find all UserProfile based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of UserProfile per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.userProfileAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.userProfileAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch userProfiles from the database
  let userProfiles = await UserProfile.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    userProfiles = await getPopulatedFields(populatedFields, userProfiles);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    userProfiles = selectFields(userProfiles, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    userProfiles = userProfiles.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await UserProfile.count(searchQuery);
  ...
  let finalItems = [];
  if (query.selectedFields.length > 0) {
    finalItems = userProfiles.map((userProfile) => ({
      id: userProfile.id,
      data: { ...userProfile },
    }));
  } else {
