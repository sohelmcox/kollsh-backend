...
/**
 * Find all Attribute based on provided query parameters.
 *
 ...
 * @param {number} params.pageSize - Number of Attribute per page for pagination.
...
const findAll = async ({
  ...
  const query = {
    sortCriteria: parseSortCriteria(sort, config.attributeAllowedSorByFields),
  ...
  if (query.searchQuery) {
    searchQuery = getSearchQuery(
      query.searchQuery,
      defaults.attributeAllowedSearchFields,
    );
  }
  const sortStr = query.sortCriteria;
  // Fetch attributes from the database
  let attributes = await Attribute.find(searchQuery)
    .sort(sortStr)
  ...
  if (populatedFields.length > 0) {
    attributes = await getPopulatedFields(populatedFields, attributes);
  }

  // Select fields
  if (query.selectedFields.length > 0) {
    attributes = selectFields(attributes, query.selectedFields);
  }
  // skip pageStart
  if (query.skip > 0) {
    attributes = attributes.slice(query.skip);
  }
  // limit totalEntities
  const totalCount = await Attribute.count(searchQuery);
  ...
  let finalItems = [];
  if (query.selectedFields.length > 0) {
    finalItems = attributes.map((attribute) => ({
      id: attribute.id,
      data: { ...attribute },
    }));
  } else {
